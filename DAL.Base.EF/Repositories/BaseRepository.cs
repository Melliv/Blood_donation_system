...
namespace DAL.Base.EF.Repositories
{
        ...
        where TDbContext : DbContext
    {
        ...
        {
            var query = CreateQuery(userId, noTracking);
            // await query.Select(d => Mapper.Map(d)).FirstOrDefaultAsync(e => e!.Id.Equals(id));
        ...
        public virtual async Task<bool> ExistsAsync(TKey id, TKey? userId = default)
        {
            ...
            if (!typeof(IDomainAppUserId<TKey>).IsAssignableFrom(typeof(TDomainEntity)))
                throw new AuthenticationException($"Entity {typeof(TDomainEntity).Name} does not implement required interface: {typeof(IDomainAppUserId<TKey>).Name} for AppUserId check");
